...
def merge_copy_additive(base: Region, patch: Region):
    """
    Additive merge: only copies chunks from patch that base does not already
    have.  Existing base chunks are never overwritten.

    Use this when base is the authoritative golden copy and patch is a client
    upload.  This guarantees the golden can only grow — a client upload can
    contribute new exploration but can never degrade merged data that is
    already stored for a chunk position.
    """
    for tx in range(8):
        for tz in range(8):
            patch_tile = patch.tile_chunks[tx][tz]
            ...
            for cx in range(4):
                for cz in range(4):
                    ...
                    if base_tile.chunks is None:
                        base_tile.chunks = [[Chunk() for _ in range(4)] for _ in range(4)]
                    # Only add if the golden doesn't already have this chunk
                    if base_tile.chunks[cx][cz].pixels is None:
                        base_tile.chunks[cx][cz] = copy.deepcopy(patch_chunk)
